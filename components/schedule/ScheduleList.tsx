      ...
      sessions
        .filter(
          (s) =>
            ...
            (!instructorId || s.instructorId === instructorId) &&
            (!first || s.firstTimerEligible) &&
            (!available || s.status === "Available"),
        )
        .slice(0, limit),
    [classId, instructorId, first, available, limit],
  );
  return (
    <div>
      <p className="placeholder">
        <strong>Demonstration schedule:</strong> dates and statuses are
        illustrative, not live availability or capacity.
      </p>
      {!limit && (
      ...
      <div aria-live="polite" className="technical">
        {list.length} demonstration sessions
      </div>
      {list.length === 0 ? (
        <p className="section">
          No sessions match these filters. Clear filters or contact the studio.
        </p>
      ) : (
        list.map((s) => {
          ...
          return (
            <article className="schedule-row" key={s.id}>
              <time dateTime={`${s.date}T${s.startTime}`}>
                ...
                <br />
                <a className="technical" href={`/api/calendar/${s.id}`}>
                  Add to calendar
                </a>
              </time>
              <div>
                <Link href={`/classes/${c.slug}`}>
                  <strong>{c.name}</strong>
                </Link>
                <div className="technical">
                  {c.duration} min · {c.intensity}
                </div>
              </div>
              <Link href={`/instructors/${i.slug}`}>{i.name}</Link>
              <span className="tag">{s.status}</span>
              {s.status === "Waitlist" ? (
                <Link className="btn outline" href={`/contact?session=${s.id}`}>
                  Join waitlist
                </Link>
              ) : s.status !== "Cancelled" ? (

...
export const bookingSchema = z.object({
  ...
  preferredClass: z.string().min(1, "Choose a class"),
  preferredDate: z.string().min(1, "Choose a date"),
  preferredTime: z.string().min(1, "Choose a time"),
  experience: z.enum(["First time", "Some experience", "Experienced"]),
  preferredInstructor: z.string().optional(),
  limitationNote: z.string().max(1000).optional(),
  consent: accepted,
  policy: z.boolean().refine(Boolean, "Confirm the booking policy"),
